...
class Ping(object):
	def __init__(self, destination, timeout=1000, packet_size=55, own_id=None, quiet_output=False, udp=False, bind=None):
	...
	def do(self):
		...
		#send_time = self.send_one_ping(current_socket,srcIP)
	...
	# def send_one_ping(self, current_socket,srcIP):
...
number_of_requests = randint(1, number_of_files)
print(number_of_requests)


for i in range(1,number_of_requests+1):
    tmp = randint(1, number_of_files)
    while tmp in requests:
        tmp = randint(1, number_of_files)

    requests.append(tmp)

des = randint(1, number_of_cars)
